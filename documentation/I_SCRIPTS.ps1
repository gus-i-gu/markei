...
function Get-RepositoryFile {
    ...
    # Validate staged state and exact worktree bytes independently. Git status
    # can report platform-specific line-ending/stat changes even when the raw
    # worktree file is byte-identical to the index.
    & git -C $RepositoryRoot diff --cached --quiet -- $RelativePath
    $StagedDiffExitCode = $LASTEXITCODE
    if ($StagedDiffExitCode -eq 1) {
        throw "Migration file has staged changes: $RelativePath"
    }
    if ($StagedDiffExitCode -ne 0) {
        throw "Unable to verify staged migration state: $RelativePath"
    }

    $IndexBlob = [string](
        & git -C $RepositoryRoot rev-parse ":$RelativePath" 2>$null
    )
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to read migration file from the Git index: $RelativePath"
    }
    $IndexBlob = $IndexBlob.Trim()

    $WorktreeBlob = [string](
        & git -C $RepositoryRoot hash-object --no-filters -- $RelativePath 2>$null
    )
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to hash migration worktree bytes: $RelativePath"
    }
    $WorktreeBlob = $WorktreeBlob.Trim()

    $GitObjectPattern = '^(?:[A-Fa-f0-9]{40}|[A-Fa-f0-9]{64})        Get-FileHash $ResolvedFile.FullName -Algorithm SHA256
...
"@
    ...
    if ($IndexBlob -notmatch $GitObjectPattern -or
        $WorktreeBlob -notmatch $GitObjectPattern) {
        throw "Git returned an invalid migration blob identifier: $RelativePath"
    }
    if ($WorktreeBlob -cne $IndexBlob) {
        throw "Migration file bytes differ from the Git index: $RelativePath"
    }

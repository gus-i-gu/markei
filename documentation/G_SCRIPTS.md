...
- Android target inventory is not exactly one ready, authorized target;
- the Markei Android package is absent, `run-as` is unavailable, force-stop
  does not hold, or remote size/SHA-256 proof is unavailable;
- the fixed Android snapshot directory already exists, binary extraction
  differs from its remote source, or its private manifest no longer matches;
- Android `integrity_check`, foreign-key, active-binding, or exact local
  Device-scope guards fail or produce an ambiguous apply-state classification.
